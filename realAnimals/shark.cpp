...
Shark::Shark(pair<int,int> _position):
       Animal("SHK", ++shark_nb, '7', 771, food, 'K', _position) {
  eat = food;
  habitat.insert('W');
...
Shark::Shark(float _weight, pair<int,int> _position):
       Animal("SHK", ++shark_nb, '7', _weight, food, 'K', _position) {
  eat = food;
  habitat.insert('W');
  compatible.insert("SHK");
  compatible.insert("WHL");
...
Shark::Shark(const Shark& s):
       Animal(s.id, s.number, s.legend, s.weight, s.eat, s.type, s.position) {
  habitat = s.habitat;
}
